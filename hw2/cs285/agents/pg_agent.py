...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = [self.gamma **indices[t] for t in range(len(indices))]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = [rewards[t] * discounts[t] for t in range(len(discounts))]
        ...
        list_of_discounted_returns = [sum_of_discounted_rewards  for t in range(len(rewards))]
